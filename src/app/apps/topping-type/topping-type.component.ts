...
export class ToppingTypeComponent implements OnInit, ListPageInterface {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.topping.getList({ query }))
    this.itemCount = this.innowayApi.topping.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.topping.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.topping.deleteAll(ids)
