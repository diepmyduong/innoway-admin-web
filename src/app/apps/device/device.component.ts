...
export class DeviceComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscriptions.onItemsChange = this.innowayApi.device.items.subscribe(items => {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.device.getList({ query }))
    this.itemCount = this.innowayApi.device.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.device.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.device.deleteAll(ids)
