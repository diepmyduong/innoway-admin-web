...
export class EmployeeTypeComponent implements OnInit, ListPageInterface {
  ...
  async getItems() {
    ...
    this.items = await this.innoway.getAll('employee_type', query);
  ...
  addItem() {
    this.router.navigate(['/employee-type/add']);
  }

  editItem(item) {
    this.router.navigate(['/employee-type/add', item.id]);
  }

  viewItem(item) {
    this.router.navigate(['/employee-type/detail', item.id]);
  }
