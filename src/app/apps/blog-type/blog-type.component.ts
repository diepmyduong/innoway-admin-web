...
export class BlogTypeComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscriptions.onItemsChange = this.innowayApi.blogType.items.subscribe(items => {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.blogType.getList({ query }))
    this.itemCount = this.innowayApi.blogType.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.blogType.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.blogType.deleteAll(ids)
