...
export class SmartCodeComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.smartCode.getList({ query }))
    this.itemCount = this.innowayApi.smartCode.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.smartCode.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.smartCode.deleteAll(ids)
