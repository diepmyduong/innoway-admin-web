...
export class BlogComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscriptions.onItemsChange = this.innowayApi.blog.items.subscribe(items => {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.blog.getList({ query }))
    this.itemCount = this.innowayApi.blog.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.blog.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.blog.deleteAll(ids)
