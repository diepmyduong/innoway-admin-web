...
export class ExportHistoryComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.exportHistory.getList({ query }))
    this.itemCount = this.innowayApi.exportHistory.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.exportHistory.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.exportHistory.deleteAll(ids)
