...
export class AddComponent implements OnInit {
  ...
  async addItem(form: NgForm) {
    if (form.valid) {
      let { name, named, type, actions, ip, brandDevice, roomId, status } = this;
      let brand_device = brandDevice
      let room_id = roomId
      await this.innowayApi.contract.add({
        name, named, type, actions,
        ip, brand_device, room_id, status
      })
  ...
  async updateItem(form: NgForm) {
    if (form.valid) {
      let { name, named, type, actions,
        ip, brandDevice, roomId, status } = this;
      let brand_device = brandDevice
      let room_id = roomId
