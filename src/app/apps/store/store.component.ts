...
export class StoreComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscriptions.onItemsChange = this.innowayApi.store.items.subscribe(items => {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.store.getList({ query }))
    this.itemCount = this.innowayApi.store.pagination.totalItems
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.store.delete(item.id)
  ...
  async deleteAll() {
    ...
    try {
      try { await this.confirmDelete() } catch (err) { return };
      await this.innowayApi.store.deleteAll(ids)
